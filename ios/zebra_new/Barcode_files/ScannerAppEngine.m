...
zt_ScannerAppEngine *_g_bsharedAppEngine = nil;

+ (zt_ScannerAppEngine *) BsharedAppEngine
{
    @synchronized([zt_ScannerAppEngine class])
    {
        if (_g_bsharedAppEngine == nil)
        {
            [[self alloc] init];
        }
        
        return _g_bsharedAppEngine;
    }
...
+(id)alloc
{
    @synchronized([zt_ScannerAppEngine class])
    {
        NSAssert(_g_bsharedAppEngine == nil, @"Attempted to allocate a second instance of a singleton.");
        _g_bsharedAppEngine = [super alloc];
        return _g_bsharedAppEngine;
    }
  ...
  /*
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIAlertView *alert = [[UIAlertView alloc]
                              initWithTitle:ZT_BarCode_APP_NAME
                              message:message
                              delegate:nil
...
- (int)showBackgroundNotification:(NSString *)text aDictionary:(NSDictionary*)param_dict
{
    ...
    if ([[UIApplication sharedApplication] applicationState] != UIApplicationStateActive)
    {
        ...
        if (notif)
        {
            ...
            notif.alertAction = ZT_BarCode_APP_NAME;
...
- (NSString*)getScannerModelName:(int)scannerModel
{
    switch (scannerModel)
    {
        ...
        case SBT_DEVMODEL_SSI_CS6080:
            return SST_SCANNER_MODEL_SSI_CS6080;
        case SBT_DEVMODEL_SSI_DS2278:
            return SST_SCANNER_MODEL_SSI_DS2278;
        case SBT_DEVMODEL_SSI_RS5100:
            return SST_SCANNER_MODEL_SSI_RS5100;
    }
    return SST_SCANNER_MODEL_UNKNOWN;
}
...
- (void)blinkLEDOff
{
    ...
    [[zt_ScannerAppEngine BsharedAppEngine] executeCommand:SBT_SET_ACTION aInXML:in_xml_LED aOutXML:nil forScanner:[[ConnectionManager sharedConnectionManager] getConnectedScannerId]];
}
...
//- (SRFID_RESULT)switchMode:(int)rfIDMode
//{
//
