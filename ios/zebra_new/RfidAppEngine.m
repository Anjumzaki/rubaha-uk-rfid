...
//  NSLog(@"eventReadNotify: tagId = %@, RSSI, %hd", [tagData getTagId], [tagData getPeakRSSI]);
//  
  ...
  for (id<zt_IRfidAppEngineTriggerEventDelegate> delegate in m_TriggerEventDelegates )
  {
      if (delegate != nil && ([[[zt_RfidAppEngine sharedAppEngine] activeReader] getBatchModeRepeat] == NO))
      {
          dispatch_async([m_RadioOperationEngine getQueue], ^{
              dispatch_async(dispatch_get_main_queue(), ^{
                  [delegate onInventoryItem:tagData];
              });
          });
      }
  }
  ...
  for (id<zt_IRfidAppEngineTriggerEventDelegate> delegate in m_TriggerEventDelegates )
    {
        if (delegate != nil && ([[[zt_RfidAppEngine sharedAppEngine] activeReader] getBatchModeRepeat] == NO))
        {
            dispatch_async([m_RadioOperationEngine getQueue], ^{
                dispatch_async(dispatch_get_main_queue(), ^{
                    [delegate getProximity:proximityPercent];
                });
            });
        }
    }
...
- (SRFID_RESULT)setAntennaConfiguration:(NSString **)statusMessage pow:(float)power
{
  SRFID_RESULT srfid_result = SRFID_RESULT_FAILURE;
  
  
  srfidAntennaConfiguration *configs = [[[srfidAntennaConfiguration alloc]init] autorelease];
  
  
  srfidAntennaConfiguration *localconfigs = [m_SledConfiguration getAntennaConfig];
  
  [configs setPower: power];
  [configs setTari:[localconfigs getTari]];
  [configs setDoSelect:[localconfigs getDoSelect]];
  [configs setLinkProfileIdx:[localconfigs getLinkProfileIdx]];
  
  for(int i = 0; i < ZT_MAX_RETRY; i++)
  {
      srfid_result = [m_RfidSdkApi srfidGetAntennaConfiguration:[m_ActiveReader getReaderID] aAntennaConfiguration:&configs aStatusMessage:statusMessage];
  ...
  if (srfid_result == SRFID_RESULT_SUCCESS)
  {
    ...
    isMatch = [m_SledConfiguration setAntennaOptionsWithPower:power];
    [m_TemporarySledConfigurationCopy setAntennaOptionsWithPower:power];
...
- (SRFID_RESULT)writeonTag:(NSString*)tagID withData:(NSString*)data
{
    if (m_RfidSdkApi != nil)
    {
      short offset = 2;
      ...
      SRFID_RESULT rfid_result = [self writeTag:tagID withTagData:&tagData withMemoryBankID:SRFID_MEMORYBANK_EPC withOffset:offset withData:data withPassword:password doBlockWrite:doBlockWrite aStatusMessage:&err_msg];
      return  rfid_result;
    }
...
#pragma mark - sdk Session Update

-(SRFID_RESULT)UpdateSingultionControl:(SRFID_SESSION)currentVal setTagPopulation:(int)populationVal setSlFlag:(SRFID_SLFLAG)SLFlag setInventoryState:(SRFID_INVENTORYSTATE)InState{
  ...
  SRFID_RESULT result = [m_RfidSdkApi srfidGetSingulationConfiguration:[m_ActiveReader getReaderID] aSingulationConfig:&singulation_cfg aStatusMessage:&error_response];
  if (SRFID_RESULT_SUCCESS == result) {
  /* singulation configuration received */
  
    /* change the received singulation configuration */
    [singulation_cfg setTagPopulation:populationVal];
    [singulation_cfg setSession:currentVal];
    [singulation_cfg setSlFlag:SLFlag];
    [singulation_cfg setInventoryState:InState];
    error_response = nil;
    /* set updated singulation configuration */
    result = [m_RfidSdkApi srfidSetSingulationConfiguration:[m_ActiveReader getReaderID] aSingulationConfig:singulation_cfg aStatusMessage:&error_response];
  }
...
#pragma mark - Set Unique Tag

-(SRFID_RESULT)setUniqueTag:(BOOL)currentVal{
  
      srfidUniqueTagsReport* setUniqueTag = [[srfidUniqueTagsReport alloc]init];
  
     [setUniqueTag setUniqueTagsReportEnabled:currentVal];
     NSString *statusMessage = nil;
     SRFID_RESULT result = [m_RfidSdkApi srfidSetUniqueTagReportConfiguration:[m_ActiveReader getReaderID] aUtrConfiguration:setUniqueTag aStatusMessage:&statusMessage];
...
- (SRFID_RESULT)switchMode:(int)barcodeMode
{

        NSString *statusMessage;
         SRFID_RESULT result;
         
         result =[m_RfidSdkApi srfidSetAttribute:m_ActiveReader.getReaderID attributeNumber:1664 attributeValue:barcodeMode attributeType:@"B" aStatusMessage:&statusMessage];
         if (SRFID_RESULT_SUCCESS == result )
         {
           return SRFID_RESULT_SUCCESS;
         }
...
-(SRFID_RESULT)UpdateBeep:(SRFID_BEEPERCONFIG)config{
  
  NSString *statusMessage;
   SRFID_RESULT result;
   
    result = [m_RfidSdkApi srfidSetBeeperConfig:[m_ActiveReader getReaderID] aBeeperConfig:config aStatusMessage:&statusMessage];
...
-(SRFID_RESULT)updatePowerManage:(bool)currentVal{

  NSString *statusMessage;
   SRFID_RESULT result;
   
  srfidDynamicPowerConfig *config = [[[srfidDynamicPowerConfig alloc] init] autorelease];
  [config setDynamicPowerOptimizationEnabled:currentVal];
  
  result = [m_RfidSdkApi srfidSetDpoConfiguration:[m_ActiveReader getReaderID] aDpoConfiguration:config aStatusMessage:&statusMessage];
              //:[m_ActiveReader getReaderID] aBeeperConfig:config aStatusMessage:&statusMessage];
...
-(SRFID_RESULT)Start_LocateTag:(NSString *)m_LocationingTagId {
  
  SRFID_RESULT result;
  NSString *status_msg = nil;
  int reader_id = [[self activeReader] getReaderID];
  result = [self sdkStartTagLocationing:reader_id aEpcId:m_LocationingTagId aStatusMessage:&status_msg];
  
  if (SRFID_RESULT_SUCCESS == result )
  {
//    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
//      [self sdkStopTagLocationing:reader_id aStatusMessage:nil];
//    });
...
-(SRFID_RESULT)Stop_LocateTag{
  SRFID_RESULT result;
  int reader_id = [[self activeReader] getReaderID];
  result =  [self sdkStopTagLocationing:reader_id aStatusMessage:nil];
  if (SRFID_RESULT_SUCCESS == result )
  {
      return SRFID_RESULT_SUCCESS;
    }
